     ...
     <sch:rule context="data:procedurestappen">
      <sch:let name="stappen">
        <xsl:for-each select="data:Procedurestap">
          ...
          <stap>
            ...
            <datum><xsl:value-of select="substring(data:voltooidOp/./string(),1,10)"/></datum>
            <xsl:choose>
              ...
              <xsl:when test="$code = '/join/id/stop/procedure/stap_022'"> <!-- Beperken vindbaarheid -->
                <uniek>1</uniek>                
                <exclusief_voor_ontwerp>1</exclusief_voor_ontwerp>
      ...
      <sch:let name="Beroep_ingesteld">
        <xsl:if test="$stappen/stap[code = '/join/id/stop/procedure/stap_018']"> <!-- Beroep(en) ingesteld -->
            <xsl:if test="not($stappen/stap[code = '/join/id/stop/procedure/stap_003'])"> <!-- Ondertekening -->
              {"code": "STOP1319",
              "soortStap1": "/join/id/stop/procedure/stap_018", 
              "soortStap2": "/join/id/stop/procedure/stap_003"}
            </xsl:if>
            <xsl:if test="not($stappen/stap[code = '/join/id/stop/procedure/stap_016'])"> <!-- Einde beroepstermijn -->
                {"code": "STOP1319",
                "soortStap1": "/join/id/stop/procedure/stap_018", 
                "soortStap2": "/join/id/stop/procedure/stap_016"}
            </xsl:if>
          </xsl:if>
      </sch:let>
      <sch:let name="Einde_beroepstermijn">
          <xsl:if test="$stappen/stap[code = '/join/id/stop/procedure/stap_016']"> <!-- Einde beroepstermijn -->
            <xsl:if test="not($stappen/stap[code = '/join/id/stop/procedure/stap_003'])"> <!-- Ondertekening -->
              {"code": "STOP1319",
              "soortStap1": "/join/id/stop/procedure/stap_016", 
              "soortStap2": "/join/id/stop/procedure/stap_003"}
            </xsl:if>
          </xsl:if>
      </sch:let>
      <sch:let name="Begin_inzagetermijn">
          <xsl:if test="$stappen/stap[code = '/join/id/stop/procedure/stap_014']"> <!-- Begin inzagetermijn -->
            <xsl:if test="not($stappen/stap[code = '/join/id/stop/procedure/stap_005'])"> <!-- Einde inzagetermijn -->
              {"code": "STOP1319",
              "soortStap1": "/join/id/stop/procedure/stap_014", 
              "soortStap2": "/join/id/stop/procedure/stap_005"}
            </xsl:if>          
          </xsl:if>
      </sch:let>
      <sch:let name="Einde_bezwaar">
          <xsl:if test="$stappen/stap[code = '/join/id/stop/procedure/stap_015']"> <!-- Einde bezwaar -->
            <xsl:if test="not($stappen/stap[code = '/join/id/stop/procedure/stap_003'])"> <!-- Ondertekening -->
              {"code": "STOP1319",
              "soortStap1": "/join/id/stop/procedure/stap_015", 
              "soortStap2": "/join/id/stop/procedure/stap_003"}
            </xsl:if>
          </xsl:if>
      </sch:let>
      <sch:let name="json_STOP1319" value="concat($Begin_inzagetermijn,$Beroep_ingesteld,$Einde_beroepstermijn,$Einde_bezwaar)"/>
      <sch:assert id="STOP1319" test="normalize-space($json_STOP1319) = ''">
      ...
      <sch:let name="json_STOP1302">
        <xsl:for-each select="$stappen/stap">
          <xsl:if test="uniek = '1' and preceding-sibling::*[1]/code = code">
            {"code": "STOP1302", 
              "soortStap": "<xsl:value-of select="code"/>", 
              "datumStap1": "<xsl:value-of select="datum"/>",
              "datumStap2": "<xsl:value-of select="preceding-sibling::*[1]/datum"/>" }
          </xsl:if>
        </xsl:for-each>
      </sch:let>
      <sch:assert id="STOP1302" test="normalize-space($json_STOP1302/./string()) = ''">
      ...
      <sch:let name="STOP1300_exclusief_voor_ontwerp">
        <xsl:for-each select="$stappen/stap">
          <xsl:if test="exclusief_voor_ontwerp = '1'">
            <xsl:value-of select="code"/><xsl:text> (voltooid op </xsl:text><xsl:value-of select="datum"/><xsl:text>) </xsl:text>
          </xsl:if>
        </xsl:for-each>
      </sch:let>
      ...
      <sch:assert id="STOP1300_definitief_ontwerp" test="not ($STOP1300_exclusief_voor_definitief != '' and $STOP1300_exclusief_voor_ontwerp != '')">
        {"code": "STOP1300", "soortStap1": "<sch:value-of select="normalize-space($STOP1300_exclusief_voor_definitief)"/>", "soortStap2": "<sch:value-of select="normalize-space($STOP1300_exclusief_voor_ontwerp)"/>"}
      </sch:assert>
        ...
        <xsl:for-each select="$stappen/stap">
          <xsl:if test="besluit_volgorde">
            <stap>
              ...
              <volgorde><xsl:value-of select="besluit_volgorde"/></volgorde>
              <lateredatum><xsl:value-of select="besluit_lateredatum"/></lateredatum>
            </stap>
          </xsl:if>
        </xsl:for-each>
      </sch:let>
      ...
      <sch:let name="json_STOP1303">
        <xsl:for-each select="$STOP1303_stappen/stap">
          <xsl:if test="(number(preceding-sibling::*[1]/volgorde) > number(volgorde) or (lateredatum != '' and datum = preceding-sibling::*[1]/datum))">
            {"code": "STOP1303",
            "soortStap1": "<xsl:value-of select="code"/>", 
            "datumStap1": "<xsl:value-of select="datum"/>", 
            "soortStap2": "<xsl:value-of select="preceding-sibling::*[1]/code"/>", 
            "datumStap2": "<xsl:value-of select="preceding-sibling::*[1]/datum"/>"}
          </xsl:if>
        </xsl:for-each>
      </sch:let>
      <sch:assert id="STOP1303" test="normalize-space($json_STOP1303/./string()) = ''">
        ...
        <xsl:for-each select="$stappen/stap">
          <xsl:if test="reactie_volgorde">
            <stap>
              <code><xsl:value-of select="code"/></code>
              <datum><xsl:value-of select="datum"/></datum>
              <volgorde><xsl:value-of select="reactie_volgorde"/></volgorde>
              <lateredatum><xsl:value-of select="reactie_lateredatum"/></lateredatum>
            </stap>
          </xsl:if>
        </xsl:for-each>
      </sch:let>
      ...
      <sch:assert id="STOP1304" test="normalize-space($json_STOP1304/./string()) = ''">
        ...
        <xsl:for-each select="$stappen/stap">
          <xsl:if test="beroep_volgorde">
            <stap>
              <code><xsl:value-of select="code"/></code>
              <datum><xsl:value-of select="datum"/></datum>
              <volgorde><xsl:value-of select="beroep_volgorde"/></volgorde>
              <lateredatum><xsl:value-of select="beroep_lateredatum"/></lateredatum>
            </stap>
          </xsl:if>
        </xsl:for-each>
      </sch:let>
      ...
      <sch:assert id="STOP1305" test="normalize-space($json_STOP1305/./string()) = ''">
        ...
        <xsl:for-each select="$stappen/stap">
          <xsl:if test="in_beroep">
            <stap>
              <code><xsl:value-of select="code"/></code>
              <datum><xsl:value-of select="datum"/></datum>
              <fase><xsl:value-of select="in_beroep"/></fase>
            </stap>
          </xsl:if>
        </xsl:for-each>
      </sch:let>
       
      <sch:let name="json_STOP1310">
         <xsl:for-each select="$STOP1310_tm_STOP1312_stappen/stap">
           <xsl:choose>             
             <xsl:when test="fase = 'start' and (preceding-sibling::*[1]/fase = 'start' or preceding-sibling::*[1]/fase = 'in') ">
               {"code": "STOP1310",
               "soortStap": "<xsl:value-of select="code"/>", 
               "datumStap": "<xsl:value-of select="datum"/>"}
             </xsl:when> 
           </xsl:choose>
         </xsl:for-each>
      </sch:let>
      <sch:assert id="STOP1310" test="normalize-space($json_STOP1310/./string()) = ''">
        <sch:value-of select="$json_STOP1310/./string()"/>
      </sch:assert>

      <sch:let name="json_STOP1311">
        <xsl:for-each select="$STOP1310_tm_STOP1312_stappen/stap">
          <xsl:choose>
            <xsl:when test="fase = 'in' and (preceding-sibling::*[1]/fase = 'einde' or not(preceding-sibling::*[1]/fase))">
               {"code": "STOP1311",
               "soortStap": "<xsl:value-of select="code"/>", 
               "datumStap": "<xsl:value-of select="datum"/>"}
            </xsl:when>
      ...
      <sch:assert id="STOP1311" test="normalize-space($json_STOP1311/./string()) = ''">
        <sch:value-of select="$json_STOP1311/./string()"/>
      </sch:assert>
       
      <sch:let name="json_STOP1312">
         <xsl:for-each select="$STOP1310_tm_STOP1312_stappen/stap">
           <xsl:choose>
      ...
      <sch:assert id="STOP1312" test="normalize-space($json_STOP1312/./string()) = ''">
        <sch:value-of select="$json_STOP1312/./string()"/>
      </sch:assert> 
        ...
        <xsl:for-each select="$stappen/stap">
          <xsl:if test="geschorst">
            <stap>
              <code><xsl:value-of select="code"/></code>
              <datum><xsl:value-of select="datum"/></datum>
              <fase><xsl:value-of select="geschorst"/></fase>
            </stap>
          </xsl:if>
        </xsl:for-each>
      </sch:let>
  
      <sch:let name="json_STOP1313">
      ...
      <sch:assert id="STOP1313" test="normalize-space($json_STOP1313/./string()) = ''">
        <sch:value-of select="$json_STOP1313/./string()"/>
      </sch:assert>
       
      <sch:let name="json_STOP1315">
         <xsl:for-each select="$STOP1313_tm_STOP1315_stappen/stap">
           <xsl:choose>
      ...
      <sch:assert id="STOP1315" test="normalize-space($json_STOP1315/./string()) = ''">
        <sch:value-of select="$json_STOP1315/./string()"/>
      </sch:assert>
