    ...
    <sch:rule context="data:Begripsrelaties">
      <sch:let name="Combinaties">
        <xsl:for-each select="data:Begripsrelatie">
          ...
          <combi>
            <xsl:value-of select="data:wId/string()"/><xsl:text>|</xsl:text><xsl:value-of select="data:catalogusEndPoint/string()"/>
          </combi>
        </xsl:for-each>
      </sch:let>
      <sch:let name="DubbeleCombinaties">
        <xsl:for-each select="$Combinaties/combi">
          <xsl:if test="preceding::combi[1] = .">
            <dubbel><xsl:value-of select="concat(substring-before(normalize-space(.),'|'), ' - ', substring-after(normalize-space(.),'|'))"/></dubbel>
          </xsl:if>
        </xsl:for-each>        
      </sch:let>
      ...
      <sch:assert id="STOP1041" test="$melding = ''">
        { "code": "STOP1041",
